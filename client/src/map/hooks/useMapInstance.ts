 ...
 * <Map> — like the one that owns its event handlers — reach the instance;
 * `current` alone would only ever resolve for descendants.
 *
 ...
 * no declarative equivalent — addImage, addProtocol, custom controls — and
 * never to mutate layers that <Layer> owns.
 */
